        ...
        try:
            ...
            if use_transcript:
                try:
                    ...
                    self.logger.info(
                        f"Fetched transcript for video {video_id}, length: {len(transcript_text)} chars"
                    )
                except (NoTranscriptFound, TranscriptsDisabled, VideoUnavailable) as e:
                    self.logger.warning(f"No usable transcript for {video_id}: {e}")
                    transcript_text = ""
                except Exception as e:
                    self.logger.error(
                        f"Unexpected error while fetching transcript for {video_id}: {e}",
                        exc_info=True,
                    )
                    transcript_text = ""
